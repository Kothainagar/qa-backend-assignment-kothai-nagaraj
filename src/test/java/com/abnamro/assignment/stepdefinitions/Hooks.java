...
public class Hooks extends BaseSetup {
    ...
    public void cleanupCreatedIssue(Scenario scenario) {
        ...
        if (createdIid == null) {
            log.info("No issue was created in scenario '{}'; nothing to clean up.",
                    scenario.getName());
            return;
        }

        String endpoint = "/projects/" + projectId + "/issues/" + createdIid;
        Map<String, String> headers = prepareHeaders("valid", token);

        log.info(
                "Cleaning up issue IID {} after scenario '{}'. Scenario status: {}",
                createdIid,
                scenario.getName(),
                scenario.getStatus()
        );
        ...
        try {
            var response = restAssuredWrapper.sendRequest(
                    Method.DELETE,
                    endpoint,
                    headers,
                    null
            ...
            if (response.statusCode() == 204) {
                ...
                log.info("Issue IID {} was already deleted; cleanup is complete.",
                        createdIid);
            } else {
                log.warn(
                        "Cleanup for issue IID {} returned unexpected status: {}",
                        createdIid,
                        response.statusCode()
                );
            }
