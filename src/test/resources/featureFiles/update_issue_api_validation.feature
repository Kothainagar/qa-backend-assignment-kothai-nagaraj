Feature: GitLab project issue update API validation
  ...
  Scenario Outline: Happy Flow - Update individual issue fields - <test>
    ...
    Examples:
      | test                     | fields            | values                        | fieldType |
      | Update title             | title             | Updated API issue             |           |
      | Update description       | description       | Updated API issue description |           |
      | Make issue confidential  | confidential      | true                          | boolean   |
      | Lock issue discussion    | discussion_locked | true                          | boolean   |
      ...
      | Assign user              | assignee_ids      | [42009381]                    | jsonarray |
      | Assign milestone by ID   | milestone_id      | 7586014                       | integer   |
      | Assign milestone by name | milestone         | API Test Milestone-1.0        |           |

  @updateIssue-HappyFlow
  Scenario Outline: Happy Flow - Update severity after converting an issue to incident - <severity>
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields     | values   |
      | issue_type | incident |
  ...
  @updateIssue-HappyFlow
  Scenario: Happy Flow - Update multiple fields in one request
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields                                                 | values                                                           | fieldType                               |
      | title\|description\|confidential\|start_date\|due_date | Updated issue\|Updated description\|true\|2026-09-10\|2026-09-20 | string\|string\|boolean\|string\|string |
    When I send the request to update the issue
    Then the issue should be updated successfully with the expected details

  @updateIssue-HappyFlow
  Scenario: Happy Flow -  Remove a label from an issue
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields | values             |
  ...
  @updateIssue-HappyFlow
  Scenario: Happy Flow - Close and reopen an issue
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields      | values |
      | state_event | close  |
  ...
  @skip
  Scenario Outline: Happy Flow - Update incident severity - <severity>
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields               | values               |
  ...
  Scenario Outline: Error Flow - Reject issue update with invalid IID - <test>
  ...
  @updateIssue-ErrorFlow
  Scenario: Error Flow - Reject an update request containing only updated_at
    Given I prepare an update issue request with below details
      | fields     | values               | iid  |
  ...
  @updateIssue-ErrorFlow
  Scenario: Error Flow - Reject updating a valid issue IID under the non-existent project ID
    Given A GitLab project issue exists in the project
    And I prepare an update issue request with below details
      | fields | values        | projectId |
      | title  | Updated issue | 999999999 |
    When I send the request to update the issue
    Then the request should be rejected with status code 404 and error message 404 Project Not Found

  @updateIssue-ErrorFlow
  Scenario Outline: Error Flow - Reject issue update with invalid authentication - <test>
    Given I prepare an update issue request with below details
